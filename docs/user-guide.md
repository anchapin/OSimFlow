...
| `--nomad-vault-secret-path` | string | none | Vault KV path holding the task-payload HMAC secret (issues #1449/#1535). When set, the worker renders `OSIMFLOW_TASK_PAYLOAD_SECRET` from Vault at allocation time via a template stanza, so the raw secret never appears in the job spec or dispatch meta. Requires `OSIMFLOW_TASK_PAYLOAD_SECRET` on the orchestrator with the same value for signing. |
| `--nomad-vault-secret-key` | string | `payload_secret` | Field name inside the Vault KV entry at `--nomad-vault-secret-path` holding the HMAC secret. KV v2 paths (containing `/data/`) read the field from the wrapped Data.data object (issues #1449/#1535). |
...
| `--kubernetes-payload-secret-ref` | string | none | Name of a pre-created Kubernetes Secret (key `OSIMFLOW_TASK_PAYLOAD_SECRET`) holding the task-payload HMAC secret. When set, the Job spec emits a `secretKeyRef` instead of a literal env value, so the raw secret never appears in the Job spec (issues #1449/#1535). Requires `OSIMFLOW_TASK_PAYLOAD_SECRET` on the orchestrator with the same value for signing. |
