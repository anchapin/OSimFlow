...
### Security (issues #1277, #1321, #1467)
...
**Minimum security baseline for non-localhost Redis:** When `--redis-url` points to a host other than `localhost`, `127.0.0.1`, `::1`, or `0.0.0.0`, the URL must use `rediss://` (TLS — mandatory since issue #1321, even when credentials are embedded) **and** satisfy one of:

1. **TLS + AUTH:** `rediss://user:pass@redis.example.com:6379`
2. **Explicit opt-out of the credentials check:** pass `require_auth=True` to `build_cache()` / `build_document_store()` when Redis authentication is handled externally (e.g. via an `AUTH` file or environment variable consumed by the Redis server, not the client). This skips only the URL-embedded-credentials check — TLS (`rediss://`) is still required.
...
# Good: TLS with external auth (require_auth=True skips only the credentials check)
cache = build_cache(db_path, redis_url="rediss://redis.example.com:6379", campaign_id=cid, require_auth=True)

# Good: localhost (single-node, no network exposure)
cache = build_cache(db_path, redis_url="redis://localhost:6379", campaign_id=cid)

# BAD: non-localhost without TLS — raises ValueError even with credentials (issue #1321)
cache = build_cache(db_path, redis_url="redis://app:secret@internal-redis:6379", campaign_id=cid)  # raises!

# BAD: TLS but no credentials and no require_auth opt-out — raises ValueError (issue #1277)
cache = build_cache(db_path, redis_url="rediss://redis.example.com:6379", campaign_id=cid)  # raises!
```

The same baseline applies to `build_document_store` (the document store factory) and the API server's `--api-redis-url` flag: since issue #1467, `create_app()` runs the same validation at rate-limiter construction and fails closed (raises `ValueError` at app creation) on an insecure URL. The rate-limit store is shared security state — per-key abuse counters — so a MITM on a plaintext connection must not be able to read or reset it.

Example URLs:

```text
redis://localhost:6379/0                        # local, no auth (loopback exempt)
rediss://user:pass@redis.example.com:6379/0     # AUTH + TLS (required for non-localhost)
rediss://redis.example.com:6379/0               # TLS + external auth (requires require_auth=True)
```
