...
| 9 | PBS / Torque | `tests/integration/test_real_pbs_campaign.py` | `OSIMFLOW_PBS_E2E=1` + `qsub`/`qstat` on PATH | `nightly` / `dispatch` (self-hosted runner — `runs-on: [self-hosted, pbs]`) | `.github/workflows/pbs-e2e.yml` | 4-artifact + run.json + executor=pbs (#351, #1020) |
| 10 | Dask-JobQueue | `tests/integration/test_real_dask_campaign.py` | `OSIMFLOW_DASK_E2E=1` + Dask scheduler reachable (`DASK_SCHEDULER_ADDRESS`) | `nightly` / `dispatch` (in-process `distributed.LocalCluster` — no external scheduler required) | `.github/workflows/dask-e2e.yml` | 4-artifact + run.json + executor=dask_jobqueue (#1020) |
| 11 | Docker Swarm | `tests/integration/test_real_docker_swarm_campaign.py` | `OSIMFLOW_DOCKER_SWARM_E2E=1` + Docker daemon reachable in Swarm mode | `nightly` / `dispatch` (single-node Swarm via `docker swarm init` in the runner) | `.github/workflows/docker-swarm-e2e.yml` | 4-artifact + run.json + executor=docker_swarm (#582, #1020) |
