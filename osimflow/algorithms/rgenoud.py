"""RgenoudAlgorithm — hybrid genetic algorithm with BFGS local search (issue #545).

Rgenoud (R's GENOUD) is a hybrid optimizer that combines a genetic
algorithm (GA) with gradient-based local search (BFGS/L-BFGS-B).  It is
particularly effective for **constraint-coupled** optimisation problems
where the objective function and constraints may be non-linear.

This implementation uses ``scipy.optimize.differential_evolution`` as the
GA backbone (same DE foundation as the Rgenoud algorithm) with
``polish=True`` which applies ``scipy.optimize.minimize(method="L-BFGS-B")``
to the best-so-far solution after each GA generation — giving us the
same GA + BFGS/L-BFGS-B hybrid behaviour that Rgenoud provides.

Migration notes
---------------
- **R/optim.rb** → use ``DifferentialEvolutionAlgorithm`` (``de``) or
  ``DualAnnealingAlgorithm`` (``dual_annealing``) which cover the same
  optimisation strategies without requiring Rserve.
- **R/diag.rb**  → sensitivity/diagnostic analysis is covered by
  ``MorrisAlgorithm`` (``morris``) and ``FAST99Algorithm`` (``fast99``)
  from the SALib-backed sensitivity analysis suite.

Workflow
--------
1. **Generation 0** — ``generate_samples()`` creates an initial LHS
   population of *n_samples* points.
2. **Generation N > 0** — ``observe()`` reads KPI files from the
   previous generation, runs one DE step (with ``polish=True`` to apply
   L-BFGS-B to the best candidate), and proposes a new population.
3. ``is_converged()`` returns ``True`` when the relative improvement
   in the objective over the last two generations drops below ``tol``.
"""
...
log = logging.getLogger("osimflow.algorithms.rgenoud")
...
class RgenoudAlgorithm(BaseAlgorithm):
    """Hybrid genetic algorithm with BFGS local search.

    Combines ``scipy.optimize.differential_evolution`` (GA backbone)
    with ``scipy.optimize.minimize(method="L-BFGS-B")`` applied to the
    best-so-far solution (``polish=True``) — equivalent to the Rgenoud
    (GENOUD) algorithm's GA + BFGS hybrid strategy.

    Iterative: ``is_iterative()`` returns ``True`` and ``is_converged()``
    checks the improvement tolerance across generations.
    ...
    def generate_samples(
        ...
        generations use the internal state (maintained via
        ...
        if self._pending_proposed_samples:
            ...
            log.info("Rgenoud proposed %d samples from explicit slot", len(samples))
            return samples_path
        ...
        if self._proposed_samples:
            ...
            samples_path.write_text(json.dumps({"samples": samples}, indent=2))
            log.info("Rgenoud proposed %d samples from optimizer state", len(samples))
            return samples_path
        ...
        try:
            ...
            raise RuntimeError("generate_rgenoud failed: initial LHS population") from exc

        samples_path.write_text(json.dumps({"samples": samples}, indent=2))
        log.info("Rgenoud generated %d initial samples", len(samples))
        return samples_path
    ...
    def _run_scipy_de_polished(
        self,
        results: list[tuple[list[float], float, float]],
    ) -> npt.NDArray[np.float64] | None:
        """Run one step of DE with BFGS-polish and return the best point, or None.

        This replicates the Rgenoud GA + BFGS hybrid: differential_evolution
        handles the global search (selection, crossover, mutation), and
        ``polish=True`` applies L-BFGS-B to the best candidate found.
        """
        ...
        try:
            # differential_evolution with polish=True applies L-BFGS-B to the
            # best solution found by the GA — this is the GA + BFGS hybrid
            # that Rgenoud implements.
            ...
            log.warning("Rgenoud scipy step failed: %s; using best-so-far", exc)
            return None

        return np.asarray(result.x, dtype=np.float64)

    def observe(self, history: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Extract KPI values from history, run DE+BFGS, propose new samples.

        Reads the KPI files from the last generation, determines the
        best-so-far point, runs one step of DE with polishing (L-BFGS-B
        local search), and proposes a new population.
        """
        ...
        if not results:
            log.warning("Rgenoud observe(): no KPI values found in history")
            return []
        ...
        # Get a candidate best point from scipy DE with polish (GA + BFGS).
        best_point = self._run_scipy_de_polished(results)
        ...
        log.info(
            "Rgenoud observe(): best_value=%.4f, proposed %d new samples",
            self._best_value,
            len(new_samples),
        )
        return new_samples
    ...
    def is_converged(self, history: list[dict[str, Any]]) -> bool:
        ...
        if converged:
            log.info(
                "Rgenoud converged: relative change %.6f < tol %.6f",
                relative_change,
                self._tol,
            )
        return converged
