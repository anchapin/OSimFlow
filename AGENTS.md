...
- **Executor** — `osimflow/executors/__init__.py` provides
  ...
  `AzureBatchExecutor`, `GoogleBatchExecutor`, `DaskJobQueueExecutor`, `KubernetesExecutor`, `DockerSwarmExecutor`, `NomadExecutor`, and `PBSExecutor` implementations.
...
| `osimflow/executors/__init__.py` | `BaseExecutor` + `LocalExecutor` + `SlurmExecutor` + `AWSBatchExecutor` + `AzureBatchExecutor` + `GoogleBatchExecutor` + `DaskJobQueueExecutor` + `KubernetesExecutor` + `DockerSwarmExecutor` + `NomadExecutor` + `PBSExecutor`. Also includes `ExecutorRegistry` singleton with `discover_plugins()` for third-party executor auto-discovery via `entry_points` group `osimflow.executors` (issue #432). |
...
- `--executor` (local / slurm / aws_batch / azure_batch / google_batch / dask_jobqueue / docker_swarm / nomad)
