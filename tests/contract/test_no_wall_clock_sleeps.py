...
_EXEMPT_MAX_CALLS: dict[str, int] = {
    ...
    # 6 deadline-bounded condition-poll loops from issues #1756 + #1799:
    # the alerting dispatcher tests park the worker inside a destination
    ...
    # without restructuring osimflow.alerting internals. Issue #1799
    # raised the budget from 3 to 6 as new test methods were added by
    # subsequent PRs without restructuring osimflow.alerting; each new
    # sleep is structurally required because there is no Event hook on
    # ``dest.received`` to ``wait()`` on.
    "tests/unit/test_alerting.py": 6,
}
