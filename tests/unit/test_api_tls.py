...
class TestAutoGeneratedApiKeyDisplay:
    """The auto-generated API key must be printed once on stderr (issue #1117).

    Since issue #1553 (SEC-001 localhost gap) auto-generation fires
    whenever no key store is configured — for both read-only and
    read-write serve. The notice goes to stderr so it does not pollute
    uvicorn's stdout access-log channel, and the format is intentionally
    one-line so operators can grep / capture it programmatically.
    """

    def test_generated_key_printed_to_stderr(self, tmp_path: Path, capsys) -> None:
        """--enable-writes without --api-key prints the actual key value to stderr."""
        parser = _build_parser()
        ...
        err = captured.err
        # The key passed to create_app must appear verbatim in stderr.
        ...
        assert api_key in err, "the generated key value must be printed to stderr"
        # Issue #1553 unified notice (one-liner, stderr, mentions --api-key).
        assert "Generated ephemeral API key for localhost serve" in err
        # Stdout must NOT carry the key — uvicorn uses stdout for access logs.
    ...
    def test_explicit_api_key_not_regenerated(self, tmp_path: Path, capsys) -> None:
        ...
        # create_app must receive the user-provided key, not a regenerated one.
        assert mock_create.call_args is not None
        assert mock_create.call_args[1]["api_key"] == "my-secret"
