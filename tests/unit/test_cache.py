...
- container_digest resolution: docker-resolved digest + offline fallback (issue #1023)
"""
...
class TestResolveImageDigest:
    ...
    _RESOLVED_REF = "docker.io/nrel/openstudio@sha256:" + "a" * 64

    def test_resolve_uses_docker_inspect_when_available(self) -> None:
        """When docker returns a RepoDigests reference, use it verbatim."""
        fake_completed = mock.Mock(
            returncode=0,
            stdout=self._RESOLVED_REF + "\n",
            stderr="",
        )
...
class TestContainerDigestFor:
    """``_container_digest_for(label)`` produces the cache-row value."""

    _LABEL = "docker.io/nrel/openstudio:3.11.0"
    _RESOLVED_REF = "docker.io/nrel/openstudio@sha256:" + "b" * 64

    def test_resolved_path_label_then_digest(self) -> None:
        """Resolved: the row stores ``<label>@<docker-repo-digest-ref>``."""
        fake_completed = mock.Mock(returncode=0, stdout=self._RESOLVED_REF + "\n", stderr="")
        with mock.patch("osimflow.cache.subprocess.run", return_value=fake_completed):
            row = _container_digest_for(self._LABEL)
        # Format: <label>@<digest-ref>. The label is preserved for display,
        # the digest-ref drives invalidation when the image is rebuilt.
