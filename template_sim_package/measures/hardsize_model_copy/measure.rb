...
# start the measure
class HardsizeModelCopy < OpenStudio::Measure::ModelMeasure
  # human readable name
  def name
    return 'Hardsize Model Copy'
  end
  ...
  def run(model, runner, user_arguments)
    ...
    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('22.1.before_hardsize_model_copy.osm')
    end
    ...
    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('22.2.after_hardsize_model_copy.osm')
    end
...
HardsizeModelCopy.new.registerWithApplication
