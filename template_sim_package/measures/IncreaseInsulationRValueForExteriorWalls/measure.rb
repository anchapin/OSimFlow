...
class IncreaseInsulationRValueForExteriorWalls < OpenStudio::Measure::ModelMeasure
  ...
  def modeler_description
    return 'Does not alter constructions that already are above the R-value requested.'
  end

  def self._original_source
    return 'https://github.com/NREL/openstudio-ee-gem/tree/66988daabc419d8eb682177cfc7314071ac7b40c/lib/measures/IncreaseInsulationRValueForExteriorWalls'
  end
  ...
  # The definition be called through this measure round to 0 or 2)
  ...
  def run(model, runner, user_arguments)
    ...
    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('6.1.before_IncreaseInsulationRValueForExteriorWalls.osm')
    end
    ...
    if (r_value < 0) || (r_value > 500)
      ...
      return false
    elsif r_value > 70 # Changed from original source.
    ...
    exterior_surface_constructions.each do |exterior_surface_construction|
      ...
      if (max_thermal_resistance >= r_value_si) && !allow_reduction
        ...
        if found_material == false
          ...
          if !new_material_massless.empty?
            new_material_massless.get.setThermalResistance(r_value_si)
          end
          new_material_airgap = new_material.to_AirGap
          if !new_material_airgap.empty?
            new_material_airgap.get.setThermalResistance(r_value_si)
          end
        end
      end
    end
    ...
    affected_area_si = 0
    # totalCost_of_affected_area = 0 # TODO: unused
    yr0_capital_totalCosts = 0 # TODO: unused as well...
    ...
    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('6.2.after_IncreaseInsulationRValueForExteriorWalls.osm')
    end
