...
class SimulationSettings < OpenStudio::Measure::ModelMeasure
  ...
  def arguments(_model)
    ...
    dst_end.setDisplayName('Daylight Savings Ends')
    ...
    # make an argument for output_diagnostics
    output_diagnostics = OpenStudio::Measure::OSArgument.makeBoolArgument('output_diagnostics', true)
    output_diagnostics.setDisplayName('Output Diagnostics?')
    output_diagnostics.setDefaultValue(false)
  ...
  def run(model, runner, user_arguments)
    ...
    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('1.1.before_simulation_settings.osm')
    end
    ...
    output_diagnostics = runner.getBoolArgumentValue('output_diagnostics', user_arguments)
    ...
    if enable_dst
      ...
      model.getRunPeriodControlDaylightSavingTime.remove
    ...
    # Add Output:Diagnostics object
    if output_diagnostics
      output_diagnostics_obj = model.getOutputDiagnostics
      output_diagnostics_obj.addKey('DisplayAllWarnings')
    end

    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('1.2.after_simulation_settings.osm')
    end
