...
class CreateBarFromBuildingTypeRatios < OpenStudio::Measure::ModelMeasure
  ...
  def get_building_subtypes(extended: false)
    # get building_types
    doe = get_doe_building_subtypes(extended:)
    deer = get_deer_building_subtypes(extended:)
  ...
  # for general public use use extended: false
  def get_doe_building_subtypes(extended: false) # rubocop:disable Lint/UnusedMethodArgument
    array = OpenStudio::StringVector.new
    array << ''
    array << 'NA'
  ...
  def get_deer_building_subtypes(extended: false) # rubocop:disable Lint/UnusedMethodArgument
    array = OpenStudio::StringVector.new
  ...
  def arguments(_model)
    ...
    # Phase 1 (2019 migration): lay out geometry / stub space types on the
    # 90.1-2019 prototype (master plan §5 Phase 1).
    ...
    ns_to_ew_ratio.setDefaultValue(2.14)
  ...
  def custom_log_messages_to_runner(runner, debug = false)
    ...
    # Patterns for known floor area mismatch messages that can occur with certain geometry
    # configurations (e.g. PrimarySchool + party walls with double-loaded corridor).
    # Individual space type mismatches (space_type_mismatch_pattern) are only downgraded to
    # warnings when the total floor area reported in the summary line is within the tolerance
    # below — confirming that the mismatches are mere redistribution effects, not real errors.
    floor_area_tolerance_pct = 0.001
    space_type_mismatch_pattern = /\A\w[\w ]* doesn't have the expected floor area \(actual [\d,.]+ ft\^2, target [\d,.]+ ft\^2\)\z/
    floor_area_sum_pattern = /\ASum of actual floor area is (?<actual>[\d.]+) ft\^2, sum of target floor area is (?<target>[\d.]+)\.\z/

    # Pre-scan to determine whether the total floor area difference is within tolerance.
    floor_area_within_tolerance = $OPENSTUDIO_LOG.logMessages.any? do |msg|
      next false unless /openstudio.*/ =~ msg.logChannel && msg.logLevel == OpenStudio::Error

      m = floor_area_sum_pattern.match(msg.logMessage)
      next false unless m

      actual = m[:actual].to_f
      target = m[:target].to_f
      target > 0 && (actual - target).abs / target * 100.0 < floor_area_tolerance_pct
    end
    ...
    $OPENSTUDIO_LOG.logMessages.each do |msg|
      ...
      if /openstudio.*/ =~ msg.logChannel # /openstudio\.model\..*/
        ...
        if msg.logLevel == OpenStudio::Info
          ...
          if msg.logMessage.include?('Initial area of other surface')
            pattern = /Initial area of other surface '.*?' (?<initial>[\d.]+) does not equal post intersection area (?<post>[\d.]+)/
            if (m = pattern.match(msg.logMessage))
              area_initial = m[:initial].to_f
              area_post = m[:post].to_f
              tolerance = 0.01 # Default in OS SDK is 0.001, which is 10cm2. Here I accept 0.01 m2
              if (area_initial - area_post).abs > tolerance
                runner.registerError("[#{msg.logChannel}] #{msg.logMessage}")
              else
                runner.registerWarning("[#{msg.logChannel}] #{msg.logMessage}")
              end
            else
              puts 'Message did not match expected pattern'
              runner.registerError("[#{msg.logChannel}] #{msg.logMessage}")
            end
          elsif floor_area_within_tolerance &&
                (space_type_mismatch_pattern.match(msg.logMessage) ||
                 floor_area_sum_pattern.match(msg.logMessage))
            # Total floor area is within 0.001% tolerance; individual space type area
            # mismatches are redistribution effects from dual-bar geometry, not real errors.
  ...
  def run(model, runner, user_arguments)
    ...
    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('3.1.before_create_bar_from_building_type_ratios_comstock.osm')
    end
    ...
    # NOTE: we do not want to fail the workflow for some of the warnings in there
    # Proposed this for inclusion in the standards gem: https://github.com/NREL/openstudio-standards/pull/2011
    custom_log_messages_to_runner(runner, debug) # TODO: this is patched
    reset_log

    if !ENV['SAVE_INTERMEDIATE'].nil?
      model.save('3.2.after_create_bar_from_building_type_ratios_comstock.osm')
    end
