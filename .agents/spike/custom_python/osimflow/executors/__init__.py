...
class SlurmExecutor(BaseExecutor):
    ...
    With `debug=True` (the default for this spike), submitit runs the job
    as a local subprocess *but still emits the exact `sbatch` command it
    would have run* in the log. This is the documented submitit pattern
    ...
    def __init__(
        ...
        # Imported lazily so the spike does not require submitit at import time
        # for users who only ever run the local executor.
        import submitit
        ...
        ex.update_parameters(
            ...
            mem_gb=mem_gb,
            time=int(time_h * 60),  # submitit wants minutes
        )
        # If debug, submitit will use DebugExecutor which runs locally.
        if debug:
            log.warning(
    ...
    def submit(
        ...
        # submitit does not let us override per-submit cpus/mem easily here
        ...
        # production. For the spike we emit the resource directive into
        # the task's tag so it appears in logs.
        if container:
...
class AWSBatchExecutor(BaseExecutor):
    """AWS Batch executor — STUB for the spike.
    ...
    This stub is intentionally thin so the spike can demonstrate the
    `Campaign` orchestrator and cache layer without an AWS account.
    """
